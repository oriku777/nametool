...
| | `post`（REST API版） | `post-browser`（ブラウザ自動操作版） |
|---|---|---|
| 認証情報 | アプリケーションパスワード（専用・失効可能） | 通常のログインパスワードそのもの |
| 安定性 | 高い（WordPress公式API） | 低め（管理画面のデザイン変更で壊れる可能性） |
| カテゴリ/タグ/アイキャッチ画像 | 対応 | 未対応（下書き保存後に手動で設定） |
| 速度 | 速い | ブラウザ起動が必要なため遅い |

**基本的には `post`（REST API版）を推奨します。** `post-browser` は、REST APIが使えない場合の代替手段として用意しています。
...
`post-browser`（ブラウザ自動操作版）を使う場合は、あわせてブラウザ本体も取得してください。

```bash
npx playwright install chromium
```

### 2. アプリケーションパスワードの発行（`post` コマンドを使う場合）
...
### ブラウザ自動操作版を使う（`post-browser`）

`.env` に `WP_PASSWORD`（通常のログインパスワード）を設定した上で:

```bash
node src/cli.js post-browser --title "テスト投稿" --content "本文をここに書きます"
```

うまく動かない場合は `--headed` を付けるとブラウザ画面が表示され、どこで止まっているか確認できます（画面のある環境でのみ使用可能）。

```bash
node src/cli.js post-browser --title "テスト投稿" --content "本文" --headed
```

**制限事項:** カテゴリ・タグ・アイキャッチ画像の指定には対応していません。下書き保存後、管理画面から手動で設定してください。また管理画面のデザインが変わるとセレクタが合わなくなり動作しなくなる可能性があります。エラーが出た場合は、エラーメッセージまたは `--headed` 実行時の画面を共有していただければ修正します。
